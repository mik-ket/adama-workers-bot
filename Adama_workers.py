...
jobs = {
    "የቤት አያያዝ ሥራዎች": ["ተመላላሽ ሰራተኛ", "ሞግዚት","አስጠኚ", "ልብስ አጣቢ", "አከራይ ደላላ", "የድግስ ምግብ አዘጋጅ"],
    "የጥገና ባለሙያ": ["የፍሪጅ ጥገና","የልብስ ማጠቢያ ማሽን ጥገና", "የTV እና ኤሌክትሮኒክስ ጥገና", "የምጣድ እና ምድጃ ጥገና","የዲሽ ተከላ እና ጥገና", "የኮምፒዩተር ጥገና", "መኪና ጥገና(መካኒክ)"],
    "የቤት እድሳት": ["ቀለም ቀቢ", "ጂብሰም ሰራተኛ", "የቧንቧ ሰራተኛ", "የኤሌክትሪክ ሰራተኛ", "አናፂ","ግንበኛ","የዝናብ መውረጃ(ቦረንዳዮ) ሰራተኛ", "ታይል ንጣፍ"],
    "የድርጅት ሥራዎች": ["ካሸሪ", "ሽያጭ", "ሼፍ", "ዲሊቨሪ", "መስተንግዶ", "እንግዳ ተቀባይ", "ፀሐፊ", "የሒሳብ ሰራተኛ"] }
...
async def start(update: Update, context: CallbackContext) -> int:
    await update.message.reply_text("👋እንኳን ደህና መጡ። ሙሉ ስም አስገቡ?(በፈለጉት ቋንቋ አስገቡ።)")
    return ASK_SEX
...
async def ask_sex_handle_name(update: Update, context: CallbackContext)-> int:
    ...
    await update.message.reply_text("ፆታ ይምረጡ?",reply_markup=reply_markup_sex)
    return ASK_PHONE
...
async def ask_specific_area(update: Update, context: CallbackContext) -> int:
    ...
    if selected_major_area not in major_areas:
        ...
        await update.message.reply_text("የምትኖሩበትን ቀበሌ/ሰፈር ምረጡ:", reply_markup=reply_markup)
        return HANDLE_PLACE
...
async def ask_jobs(update: Update, context: CallbackContext) -> int:
    ...
    await query.edit_message_text(
        text="በ መረጡት ዘርፍ ውስጥ እስከ 3 የስራ አይነቶችን ይምረጡ:",
...
async def select_job(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if selected_job in user_selected_jobs[user_id]:
        ...
        if len(user_selected_jobs[user_id]) < 3:
            ...
            await query.answer("መምረጥ የሚችሉት 3ሙያ ብቻ ነው።", show_alert=True)
            return SELECTED_JOBS 
...
async def confirm_selection(update: Update, context: CallbackContext) -> int:
    ...
    if not user_selected_jobs[user_id]:
        await query.answer("ቢያንስ 1የሙያ አይነት ይምረጡ።", show_alert=True)
        return SELECTED_JOBS
